...
notOfInterest=pinch_signal(rows_rest1);
h = spectrum.welch; % creates the Welch spectrum estimator
SNOIf=psd(h,notOfInterest,'Fs',fs); % calculates and plot the one sided PSD
plot(SNOIf); % Plot the one-sided PSD.

legend("VF", "Flex", "Pinch", "Rest");
temp=get(gca);
...
xlabel('Time (s)')
ylabel('Amplitude (uV)')
title('Filtered VF signal with labels for stimulation periods')
